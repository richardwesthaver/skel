...
#+RESULTS:
#+begin_src lisp
; compiling file "/home/ellis/dev/skel/tests.lisp" (written 27 SEP 2023 07:42:03 PM):

; wrote /home/ellis/dev/skel/tests.fasl
; compilation finished in 0:00:00.010
in suite SKEL with 5/5 tests:
:DEBUG @ 94.520004 :: running test:  #<TEST VM :fn VM-test2324 :args NIL :persist NIL {1006F848B3}> 
:DEBUG @ 94.526665 :: #<PASS (LET ((VM (MAKE-SK-VM C9)))
                               (DOTIMES (I C8) (SKS-POP VM))
                               T)> 
:DEBUG @ 94.526665 :: #<PASS (SKS-POP VM)> 
#<PASS VM-TEST2324> 
:DEBUG @ 94.526665 :: running test:  #<TEST MAKEFILE :fn MAKEFILE-test2323 :args NIL :persist NIL {1006F82D23}> 
:DEBUG @ 94.55 :: #<PASS (NULL
                          (SK-WRITE-FILE (MK) IF-EXISTS SUPERSEDE PATH
                                         ,*DEFAULT-MAKEFILE*))> 
:DEBUG @ 94.56 :: #<PASS (PUSH-RULE R1 MK1)> 
:DEBUG @ 94.56 :: #<PASS (PUSH-RULE R2 MK1)> 
:DEBUG @ 94.56 :: #<PASS (PUSH-DIRECTIVE
                          (CMD ifeq ($(DEBUG),1) echo foo 
endif)
                          MK1)> 
:DEBUG @ 94.56 :: #<PASS (PUSH-VAR '(A B) MK1)> 
:DEBUG @ 94.56 :: #<PASS (PUSH-VAR '(B C) MK1)> 
:DEBUG @ 94.56 :: #<PASS (NULL
                          (SK-WRITE-FILE MK1 IF-EXISTS SUPERSEDE PATH test.mk))> 
#<PASS MAKEFILE-TEST2323> 
:DEBUG @ 94.56 :: running test:  #<TEST SKELFILE :fn SKELFILE-test2322 :args NIL :persist NIL {1006F362A3}> 
:DEBUG @ 94.57667 :: #<PASS (SK-WRITE-FILE
                             (MAKE-INSTANCE 'SK-PROJECT NAME nada PATH %TMP)
                             IF-EXISTS OVERWRITE)> 
:DEBUG @ 94.57667 :: #<PASS (INIT-SKELFILE %TMP)> 
:DEBUG @ 94.57667 :: #<PASS (LOAD-SKELFILE %TMP)> 
:DEBUG @ 94.57667 :: #<PASS (BUILD-AST
                             (SK-READ-FILE (MAKE-INSTANCE 'SK-PROJECT) PATH
                                           %TMP))> 
#<PASS SKELFILE-TEST2322> 
:DEBUG @ 94.57667 :: running test:  #<TEST HEADER-COMMENTS :fn HEADER-COMMENTS-test2321 :args NIL :persist NIL {1006F34273}> 
:DEBUG @ 94.58 :: #<PASS (EQ
                          (TYPE-OF
...
:DEBUG @ 94.58 :: #<PASS (EQ
                          ...
                          'FILE-HEADER)> 
#<PASS HEADER-COMMENTS-TEST2321> 
:DEBUG @ 94.58 :: running test:  #<TEST SANITY :fn SANITY-test2320 :args NIL :persist NIL {1006F326E3}> 
:DEBUG @ 94.583336 :: #<PASS (EQ T (APPLY #'/= (SKELS 3E8)))> 
#<PASS SANITY-TEST2320> 
No tests failed.
#+end_src
